...
/* Shared anonymous swap pressure and fork visibility regression test. */
...
{
    return UINT64_C(0x3c6ef372fe94f82b) ^ ((uint64_t)page * UINT64_C(0x9e3779b97f4a7c15)) ^
    ...
    for (size_t step = 0; step < page_count; step++) {
        ...
        for (unsigned int marker = 0; marker < MARKER_COUNT; marker++) {
            ...
            if (actual != expected) {
                fprintf(
                    stderr,
                    "shared_anonymous_swap: mismatch page=%zu marker=%u generation=%u expected=%016" PRIx64
                    " actual=%016" PRIx64 "\n",
                    page,
                    marker,
                    generation,
...
int main(int argc, char **argv)
{
    ...
    if (page_size < sizeof(uint64_t) || page_size % sizeof(uint64_t) != 0) {
        fprintf(stderr, "shared_anonymous_swap: unsupported page size %zu\n", page_size);
        return 1;
    }
    ...
    if (argc == 2) {
        ...
        if (memory_size > SIZE_MAX - pressure_size) {
            fprintf(stderr, "shared_anonymous_swap: region size overflow\n");
            return 1;
        }
    ...
    printf(
        "shared_anonymous_swap: MemTotal=%zu MiB region=%zu MiB pages=%zu page_size=%zu\n",
        memory_size / (1024UL * 1024UL),
        region_size / (1024UL * 1024UL),
        page_count,
        page_size);

    mapping = mmap(NULL, region_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (mapping == MAP_FAILED) {
        fprintf(stderr, "shared_anonymous_swap: mmap failed: %s\n", strerror(errno));
        return 1;
    }
    ...
    pid = fork();
    if (pid < 0) {
        fprintf(stderr, "shared_anonymous_swap: fork failed: %s\n", strerror(errno));
    ...
    if (waitpid(pid, &status, 0) < 0) {
        fprintf(stderr, "shared_anonymous_swap: waitpid failed: %s\n", strerror(errno));
        munmap(mapping, region_size);
        return 1;
    }
    if (!WIFEXITED(status) || WEXITSTATUS(status) != 0) {
        fprintf(stderr, "shared_anonymous_swap: child failed status=0x%x\n", status);
        munmap(mapping, region_size);
        return 1;
    }
    ...
    if (munmap(mapping, region_size) != 0) {
        fprintf(stderr, "shared_anonymous_swap: munmap failed: %s\n", strerror(errno));
        return 1;
    }

    puts("shared_anonymous_swap: PASS");
    return 0;
}
