...
bool Bus::map_area(const Kvm &kvm, std::uintptr_t guest_addr, std::uintptr_t length, std::uint8_t **mapped_host_addr) {
    ...
    if (area_count_ == MAX_AREAS) {
        std::fprintf(stderr, "bus map area table is full\n");
        return false;
    }

    for (std::size_t i = 0; i < area_count_; i++) {
        if (ranges_overlap(guest_addr, length, areas_[i].guest_addr, areas_[i].length)) {
            std::fprintf(stderr, "bus map area overlaps existing range: new=[0x%lx,0x%lx) old=[0x%lx,0x%lx)\n",
                         ...
                         static_cast<unsigned long>(areas_[i].guest_addr),
                         static_cast<unsigned long>(areas_[i].guest_addr + areas_[i].length));
            return false;
        }
    }
    for (std::size_t i = 0; i < mmio_device_count_; i++) {
        if (ranges_overlap(guest_addr, length, mmio_regions_[i].guest_addr, mmio_regions_[i].length)) {
    ...
    areas_[area_count_] = {guest_addr, length, host_addr};
    area_count_++;
...
bool Bus::add_mmio_device(std::uintptr_t guest_addr, std::uintptr_t length, std::shared_ptr<MmioDevice> device,
    ...
    if (mmio_device_count_ == MAX_MMIO_DEVICES) {
    ...
    mmio_regions_[mmio_device_count_] = {guest_addr, length, id, std::move(device)};
    mmio_device_count_++;
    return true;
}

bool Bus::read_mmio(std::uintptr_t guest_addr, std::size_t width, std::uint64_t *value) const {
    const MmioRegion *region = find_mmio_region(guest_addr, static_cast<std::uintptr_t>(width));
...
std::uint8_t *Bus::translate(std::uintptr_t guest_addr, std::uintptr_t length) const {
    const Area *area = find_area(guest_addr, length);
    if (area == nullptr) {
        return nullptr;
    }
...
const Bus::MmioRegion *Bus::find_mmio_region(std::uintptr_t guest_addr, std::uintptr_t length) const {
    ...
    for (std::size_t i = 0; i < mmio_device_count_; i++) {
        const MmioRegion &region = mmio_regions_[i];
...
void Bus::update() {
    for (std::size_t i = 0; i < mmio_device_count_; i++) {
        if (mmio_regions_[i].device != nullptr) {
            mmio_regions_[i].device->update();
        }
    }
}
