...
uintptr_t __riscv_map_kaddr(uintptr_t kaddr_offset, uintptr_t memory_top) {
    ...
    *__riscv_init_load_kpgtable_root() = root + *__riscv_init_load_kaddr_offset();
    ...
    end = (uintptr_t)__init_end - kaddr_offset;
    for (uintptr_t kaddr = (uintptr_t)__init_start - kaddr_offset; kaddr < end; kaddr += PGSIZE) {
        map(root, kaddr, kaddr, PTE_V | PTE_R | PTE_W | PTE_X | PTE_G | PTE_A | PTE_D);
    }
