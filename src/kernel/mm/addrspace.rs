...
use super::vdso;
use super::{MapPerm, MemAccessType};
...
impl AddrSpace {
    ...
    pub fn fork(&self) -> AddrSpace {
        ...
        let new_map_manager = self
            .map_manager
            .lock()
            .fork(&self.pagetable, &new_pagetable);
    ...
    pub fn alloc_usercontext_page(&self) -> (usize, *mut UserContext) {
        ...
        self.pagetable
            .write()
            .mmap(uaddr, kaddr, MapPerm::R | MapPerm::W);
    ...
    pub fn translate_write(&self, uaddr: usize) -> SysResult<usize> {
        self.map_manager
            .lock()
            .translate_write(uaddr, &self.pagetable)
            .ok_or(Errno::EFAULT)
    }
    ...
    pub fn copy_to_user_array<T, const N: usize>(
        &self,
        uaddr: usize,
        array: &[T; N],
    ) -> SysResult<()> {
        let buffer = unsafe {
            core::slice::from_raw_parts(array.as_ptr() as *const u8, N * core::mem::size_of::<T>())
        };
        self.copy_to_user_buffer(uaddr, buffer)
    }
    ...
    pub fn copy_from_user_buffer(&self, mut uaddr: usize, buffer: &mut [u8]) -> Result<(), Errno> {
        ...
        while left > 0 {
            let kaddr = map_manager
                .translate_read(uaddr, &self.pagetable)
                .ok_or(Errno::EFAULT)?;
    ...
    pub fn copy_from_user_array<T, const N: usize>(
        &self,
        uaddr: usize,
        array: &mut [T; N],
    ) -> SysResult<()> {
    ...
    pub fn get_user_string(&self, mut uaddr: usize) -> Result<String, Errno> {
        ...
        loop {
            ...
            if let Some(pos) = slice.iter().position(|&b| b == 0) {
                result.push_str(
                    &String::from_utf8(slice[..pos].to_vec()).map_err(|_| Errno::EINVAL)?,
                );
                break;
            } else {
