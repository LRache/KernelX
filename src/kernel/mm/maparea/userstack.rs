...
impl UserStack {
    pub fn new() -> Self {
        let frames =
            Vec::from_iter((0..config::USER_STACK_PAGE_COUNT_MAX).map(|_| Frame::Unallocated));
        Self { frames }
    }
...
impl Area for UserStack {
    fn translate_read(&mut self, vaddr: usize, pagetable: &RwLock<PageTable>) -> Option<usize> {
        ...
        if page_index < self.get_max_page_count() {
            let page = match &self.frames[page_index] {
                Frame::Unallocated => self.allocate_page(page_index, &mut pagetable.write()),
    ...
    fn translate_write(&mut self, vaddr: usize, pagetable: &RwLock<PageTable>) -> Option<usize> {
        ...
        if page_index < self.get_max_page_count() {
            let page = match &self.frames[page_index] {
                ...
                Frame::Cow(_) => self.copy_on_write_page(page_index, &mut pagetable.write()),
            };
    ...
    fn try_to_fix_memory_fault(
        ...
        ktrace!(
            "UserStack::try_to_fix_memory_fault: addr={:#x}, access_type={:?}, frames={:x?}",
            addr,
            access_type,
            self.frames
        );
        ...
        match &self.frames[page_index] {
            Frame::Allocated(_) => {
                panic!(
                    "Page at index {} is already allocated, addr={:#x}, flags={:?}",
                    page_index,
                    addr,
                    pagetable.read().mapped_flag(addr)
                );
