...
cfg_if::cfg_if! {
    if #[cfg(feature="swap-memory")] {
        ...
        pub type PageFrame = swappable::AnonymousFrame;
    } else {
        ...
        impl PageFrame {
            pub fn alloc(uaddr: usize, _addrspace: &AddrSpace) -> (Self, usize) {
            ...
            pub fn copy(&self, _addrspace: &AddrSpace) -> (PageFrame, usize) {
                let new_frame = self.frame.copy();
                let kpage = new_frame.get_page();
                (PageFrame { frame: new_frame, uaddr: self.uaddr }, kpage)
            }
...
impl FrameState {
    ...
    pub fn allocate(uaddr: usize, addrspace: &AddrSpace) -> (FrameState, usize) {
        let (frame, kpage) = PageFrame::alloc(uaddr, addrspace);
