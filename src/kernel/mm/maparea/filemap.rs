...
impl FileMapArea {
    ...
    fn copy_on_write_page(&mut self, page_index: usize, pagetable: &RwLock<PageTable>) -> usize {
        ...
        if !self.perm.contains(MapPerm::W) {
            panic!(
                "Attempting copy-on-write on a non-writable mapping at page index {}",
                page_index
            );
        }
        ...
        pagetable
            .write()
            .mmap_replace(self.ubase + area_offset, page, self.perm);
        self.frames[page_index] = Frame::Allocated(Arc::new(frame));
    ...
    fn write_back_page(&self, page_index: usize) -> Result<(), crate::kernel::errno::Errno> {
        if let Frame::Allocated(frame) = &self.frames[page_index] {
            ...
            if area_offset < self.file_length {
                ...
                let length = core::cmp::min(self.file_length - area_offset, arch::PGSIZE);
                let buffer =
                    unsafe { core::slice::from_raw_parts(frame.get_page() as *const u8, length) };
                ...
                ktrace!(
                    "Wrote back page {} to file at offset {:#x}, length: {}",
                    page_index,
                    file_offset,
                    length
                );
...
impl Area for FileMapArea {
        ...
        if page_index < self.frames.len() {
            match &self.frames[page_index] {
                ...
                Frame::Allocated(_) => {
                    ...
                    panic!(
                        "Memory fault on already allocated file page at address: {:#x}",
                        uaddr
                    );
                }
                Frame::Cow(_) => {
                    assert!(
                        ...
                        "Memory fault on CoW file page for read access at address: {:#x}",
                        uaddr
                    );
                    self.copy_on_write_page(page_index, pagetable);
                }
            ...
            kdebug!(
                "Memory fault at address {:#x} for file mapping, but page index {} is out of bounds",
                uaddr,
                page_index
            );
            false
    ...
    fn split(mut self: Box<Self>, uaddr: usize) -> (Box<dyn Area>, Box<dyn Area>) {
        ...
        assert!(
            uaddr > self.ubase,
            "Split address must be greater than ubase"
        );
        assert!(
            uaddr < self.ubase + self.size(),
            "Split address out of bounds"
        );
    ...
    fn set_perm(&mut self, perm: MapPerm, pagetable: &RwLock<PageTable>) {
        ...
        for (page_index, frame) in self.frames.iter().enumerate() {
            if let Frame::Allocated(frame) = frame {
                ...
                ktrace!(
                    "Updated page table permissions for file mapping page at {:#x} to {:?}",
                    vaddr,
                    perm
                );
            }
