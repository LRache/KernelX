...
pub fn fcntl64(fd: usize, cmd: usize, arg: usize) -> Result<usize, Errno> {
    Ok(0)
}
...
pub fn openat(dirfd: usize, user_filename: usize, _flags: usize, _mode: usize) -> Result<usize, Errno> {
    ...
    let file = vfs::openat(current::pwd(), &path, flags)?;
    let file = Arc::new(file);
    let fd = current::fdtable().push(file)?;
...
pub fn fstatat(_dirfd: usize, uptr_path: usize, uptr_statbuf: usize, _flags: usize) -> Result<usize, Errno> {
    let path = current::get_user_string(uptr_path)?;

    let file = vfs::open(&path, FileFlags::dontcare()).map_err(|_| {
        ktrace!("Failed to open file for fstatat: {}", path);
        Errno::ENOENT
    })?;
