...
use crate::kernel::task::get_init_process;
...
impl PCB {
    pub fn new(pid: i32) -> Arc<Self> {
        Arc::new(Self {
            ...
            // addrspace: Arc::new(RwLock::new(addrspace)),
            // fd_table,
    ...
    pub fn new_initprocess(file: File, argv: &[&str], envp: &[&str]) -> Result<Arc<Self>, Errno> {
    ...
    // pub fn fork(self: &Arc<Self>, tcb: &Arc<TCB>) -> Result<i32, Errno> {
    //     let new_tid = tid::alloc();
    //     let new_pcb = PCB::new(new_tid);

    //     let first_task = tcb.fork(new_tid, &new_pcb);

    //     self.children.lock().push(new_pcb.clone());
    //     new_pcb.tasks.lock().push(first_task.clone());
    ...
    pub fn clone_task(
        ...
        if flags.thread {
            ...
            let new_parent = PCB::new(new_tid);
    ...
    pub fn exit(self: &Arc<Self>, code: u8) {
        ...
        *self.exit_code.lock() = code;
        ...
        while let Some(child) = children.pop() {
            get_init_process().children.lock().push(child);
        }
    }
